...
public class SnakeHeadController : MonoBehaviour
{
    ...
    public void OnMove()
    {
        HeadPos = gameObject.transform.position; //获取头的位置
        gameObject.transform.position = new Vector3(HeadPos.x + X, HeadPos.y + Y, HeadPos.z); //移动
        if (bodyList.Count > 0)
        {
            for (int i = bodyList.Count - 2; i >= 0; i--) //从后往前开始移动蛇身
            {
                bodyList[i + 1].position = bodyList[i].position; //每一个蛇身都移动到它前面一个节点的位置
            }
            bodyList[0].position = HeadPos; //第一个蛇身移动到蛇头移动前的位置
        }
    }
    //设置偏移量
